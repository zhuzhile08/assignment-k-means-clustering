...
public:
	...
	constexpr iterator erase(const_iterator pos) noexcept {
		ASSERT(pos != m_array.end(), "lstd::UnorderedSparseSet::erase(): The end iterator was passed to the function!");
	...
	template <class K> NODISCARD constexpr value_type& at(const K& key) {
		auto it = find(key);
		if (it == m_array.end()) throw std::out_of_range("lstd::UnorderedSparseSet::at(): Specified key could not be found in container!");
		return *it;
	}
