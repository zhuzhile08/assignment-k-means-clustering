...
public:
	...
	constexpr iterator erase(const_iterator pos) noexcept {
		ASSERT(pos != m_array.end(), "lstd::UnorderedSparseMap::erase(): The end iterator was passed to the function!");
	...
	constexpr value_type extract(const_iterator pos) noexcept {
		ASSERT(pos < m_array.end(), "lstd::UnorderedSparseMap::extract(): The end iterator was passed to the function!");
	...
	template <class K> NODISCARD constexpr mapped_type& at(const K& key)
		requires(!std::is_convertible_v<K, iterator> && !std::is_convertible_v<K, const_iterator>) {
		auto it = find(key);
		if (it == m_array.end()) throw std::out_of_range("lstd::UnorderedSparseMap::at(): Specified key could not be found in container!");
		return it->second;
	}
