...
public:
	...
	static_assert(isHashMapValue<container> && std::is_same_v<typename container::value_type, smart_pointer>, "lstd::BasicNode: Container template parameter only supports key-only hash maps. Please make sure the container is a hash map and has the type alias hasher defined!");
