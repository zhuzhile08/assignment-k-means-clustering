...
template <class Ty, size_type Size> struct Array {
	static_assert(Size != 0, "lstd::Array: A zero length array is forbidden!");
	...
	NODISCARD constexpr reference operator[](size_type index) noexcept {
		ASSERT(index < Size, "lstd::Array::operator[]: Index of: {} exceded array size of: {}!", index, Size);
	...
	DEPRECATED NODISCARD constexpr reference at(size_type index) {
		if (index < Size) throw std::out_of_range("lstd::Array::at: index exceeded array size!");
		return m_array[index];
	}
